...
const Page = () => {
  ...
  const fetchComptables = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://localhost:8000/api/comptables/", authHeader);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      setLoading(true);
      await axios.post("http://localhost:8000/api/comptables/", formData, authHeader);
      setFormData({
        username: "",
        first_name: "",
        last_name: "",
  ...
  const toggleActive = async (id: number, currentState: boolean) => {
    try {
      await axios.patch(
        `http://localhost:8000/api/comptables/${id}/`,
