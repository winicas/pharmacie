...
const Page = () => {
  ...
  const handleInputChange = (
    produitId: number,
    field: 'prix_achat' | 'nombre_plaquettes_par_boite',
    value: number
  ) => {
      ...
      fetch(url, {
        ...
        .then(() => {
          ...
          if (updates.nombre_plaquettes_par_boite !== undefined) {
            messages.push(
              `📦 Plaquettes/boîte : <b>${produit?.nombre_plaquettes_par_boite}</b> → <b>${updates.nombre_plaquettes_par_boite}</b>`
            )
          }
    ...
    <>
      ...
      <main className="p-6 md:p-10 bg-gradient-to-br from-blue-50 to-white min-h-screen">
        <div className="max-w-5xl mx-auto">
          {message && (
            <div
              className={`mb-6 p-4 rounded-md shadow transition-all border ${
          ...
          <div className="space-y-6">
            {produits.length === 0 ? (
              ...
              produits.map((produit) => (
                <div key={produit.id} className="bg-white p-5 rounded-xl shadow hover:shadow-md transition">
                  <h3 className="text-lg font-bold text-blue-800 mb-3">{produit.nom}</h3>
